...
Namespace ActiveRecord
	...
	Partial Public MustInherit Class Entity
		...
		''' </summary>
		Protected Overridable Function getInitialData() As Dictionary(Of String, Object)
			Return Me._initialData
		End Function
		''' <summary>
		''' Returns reserve store to store anything what has been not specified by any property or field.
		''' </summary>
		Protected Overridable Function getReserveStore() As Dictionary(Of String, Object)
		...
		''' <returns></returns>
		Default Public Overridable Property Item(memberName As String, Optional asInitialData As Boolean = False) As Object
			Get
		...
		''' <returns></returns>
		Public Overridable Function [Get](ByVal memberName As String, Optional asInitialData As Boolean = False, Optional ByRef memberInfo As Databasic.MemberInfo? = Nothing) As Object
			If asInitialData Then
				Return If(Me.getInitialData().ContainsKey(memberName), Me.getInitialData()(memberName), Nothing)
			End If
			...
			If memberInfo.HasValue Then
				...
				End If
			End If
			If Me.getReserveStore().ContainsKey(memberName) Then Return Me.getReserveStore()(memberName)
			Return Nothing
		End Function
		''' <summary>
		...
		''' <param name="memberInfo">Databasic class member info, optional.</param>
		Public Overridable Sub [Set](ByVal memberName As String, ByVal value As Object, Optional asInitialData As Boolean = False, Optional ByRef memberInfo As Databasic.MemberInfo? = Nothing)
			If Not memberInfo.HasValue Then memberInfo = MetaDescriptor.GetColumnByCodeName(Me.GetType(), memberName)
			...
			If Not assigned Then
				...
				Me.getReserveStore().Item(memberName) = value
			End If
			If asInitialData Then
				If Me.getInitialData().ContainsKey(targetValue) Then
					Me.getInitialData()(memberName) = targetValue
				Else
					Me.getInitialData().Add(memberName, targetValue)
				End If
			End If
		End Sub
		''' <summary>
		''' DynamicObject overloading method implementing reading nondeclared values from Me.getReserveStore().
		''' </summary>
		''' <param name="binder"></param>
		''' <param name="result"></param>
		''' <returns></returns>
		Public Overrides Function TryGetMember(ByVal binder As GetMemberBinder, ByRef result As Object) As Boolean
			Me.getReserveStore().TryGetValue(binder.Name, result)
			Return True
		End Function
		''' <summary>
		''' DynamicObject overloading method implementing writing nondeclared values into Me.getReserveStore().
		''' </summary>
		''' <param name="binder"></param>
		''' <param name="value"></param>
		''' <returns></returns>
		Public Overrides Function TrySetMember(ByVal binder As SetMemberBinder, ByVal value As Object) As Boolean
			If Me.getReserveStore().ContainsKey(binder.Name) Then
				Me.getReserveStore()(binder.Name) = value
			Else
				Me.getReserveStore().Add(binder.Name, value)
			End If
			Return True
		End Function
